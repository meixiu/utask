...
func (c *ChanClient) Start() error {
	...
	stealTimer := time.NewTimer(0) //窃取上次关闭未完成任务和超时任务 计时器
	...
	for {
		...
		case <-stealTimer.C:
			async(func() {
				defer func() {
					stealTimer.Reset(c.interval)
				}()
				log.Info("client steal info")
				_, err := c.Steal()
				if err != nil {
					log.Error("client steal error: ", err)
					return
				}
			}, nil)
...
func (c *ChanClient) Stop(ctx context.Context) error {
	...
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		case item := <-c.waits:
			async(func() { _ = c.Reset(item, true) }, tempProcess)
...
// Reset 重置下次处理时间
func (c *ChanClient) Reset(item task.Tasker, forStop bool) (err error) {
	id := c.id
	//设置下次处理时间
	if forStop {
		if s, ok := c.processStore.(store.StealProcessStorer); ok {
			_, err = s.Mark(c.id, item)
			return err
		}
	} else {
		item.IncreaseTimes()
	}
	_, err = c.processStore.Update(id, item)
	if err != nil {
		return err
	}
	return nil
...
func (c *ChanClient) Dispose(item task.Tasker) (err error) {
	...
	c.monitor.HandleTask(c.id, item)
	_ = c.Log(item)
	if err != nil {
		log.Error("client dispose run err: ", err, item)
		resetErr := c.Reset(item, false)
...
// Steal 窃取超时任务和上次未处理任务
func (c *ChanClient) Steal() (int64, error) {
	if s, ok := c.processStore.(store.StealProcessStorer); ok {
		return s.Steal(c.id, StealProcessStoreSize)
	}
	return 0, nil
}
