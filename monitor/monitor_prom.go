...
func (prom *PromMonitor) HandleTask(cid string, duration time.Duration, task task.Tasker) {
	status := 1
	...
	if duration != 0 {
		prom.HandleTaskCounterVec.WithLabelValues(task.GetAppID(), task.GetSID(), cid, task.GetType(), strconv.Itoa(status)).Add(float64(duration))
	}
}
...
func (prom *PromMonitor) Request(appID, processType string) {
	prom.RequestCounterVer.WithLabelValues(appID, processType).Inc()
}

// Latency producer process latency
func (prom *PromMonitor) Latency(appID, processType string, duration time.Duration) {
	prom.ResponseLatency.WithLabelValues(appID, processType).Add(float64(duration))
}
